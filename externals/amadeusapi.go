...
// TODO quanto di max?
// TODO decidere cosa ritornare e farlo ovunque: nil o lista di lista vuote
func GetFlights(origin, destination string, date time.Time, isOutbound bool) ([][]model.Segment, error) {
	...
	params.Add("adults", "1")
	params.Add("max", "3") // TODO max 3 o quanti risultati ?????
	...
	for _, flightOffer := range response.Data {
		...
		if len(flightOffer.Itineraries) == 0 {
			return flights, fmt.Errorf("no itineraries found")
		}
		...
		for _, flightSegment := range itinerary.Segments {
			...
			segment := model.Segment{
				...
				CO2Emitted: internals.ComputeAircraftEmission(int(duration.Hours()), int(duration.Minutes())), // TODO warning
				Distance:   distance,
