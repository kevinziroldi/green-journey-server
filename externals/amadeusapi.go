...
func GetFlights(originName, destinationName string, originLatitude, originLongitude, destinationLatitude, destinationLongitude float64, date time.Time, isOutbound bool) ([][]model.Segment, error) {
	...
	destinationCity, err := getOrCreateCityWithIata(destinationName, destinationLatitude, destinationLongitude)
	if err != nil {
	...
	for _, flightOffer := range response.Data {
		...
		for _, flightSegment := range itinerary.Segments {
			...
			// get cities
			segmentDepCity, err1 := GetCityFromIATA(flightSegment.Departure.IataCode)
			if err1 != nil {
				// the current flight must be discarded
				addToFlights = false
				break
			}
			segmentDestCity, err1 := GetCityFromIATA(flightSegment.Arrival.IataCode)
			if err1 != nil {
				// the current flight must be discarded
				addToFlights = false
				break
			...
			segment := model.Segment{
				...
				Departure:     segmentDepCity.CityName,
				Destination:   segmentDestCity.CityName,
				Date:          parsedTime,
				Duration:      duration,
				Vehicle:       "plane",
...
// TODO forse da mettere in city DAO!
func getOrCreateCityWithIata(name string, latitude, longitude float64) (model.City, error) {
	// TODO
	// controlla se esiste una città con questo nome nel db e con lo iata!
	// se non c'è, controlla  se c'è una città con latitudine e longitudine nel delta con iata!
	// se non c'è chiama la api
	// se la chiamata api da errore, ritorno errore (non ho lo iata)
	// altrimenti salvo nel db, se c'era senza iata, aggiorno quello!
	...
	locationResponse, err := getCityIataApiResponse(name)
	if err != nil {
	...
	if locationResponse.Data == nil ||
		...
		return model.City{}, fmt.Errorf("no data in the response")
	}
	...
	city = model.City{
		...
		Iata:        locationResponse.Data[0].IataCode,
...
func GetCityFromIATA(iata string) (model.City, error) {
	cityDAO := db.NewCityDAO(db.GetDB())

	city, err := cityDAO.GetCityByIata(iata)
	if err == nil {
		return city, nil
	}
	...
	locationResponse, err := getCityIataApiResponse(iata)
	if err != nil {
...
func getCityIataApiResponse(keyword string) (LocationResponse, error) {
